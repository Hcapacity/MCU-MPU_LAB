...
// phải có tên tag trong struct thì nó mới biết được để dùng ở bên trong
typedef struct TIM_ITR_Node{
	uint8_t flag;
	uint16_t duration;
...
void Add_TIM_ITR_Node(TIM_ITR_Node* head, TIM_ITR_Node* newNode){
	...
	// Add at other positions
		...
		while(iterator != NULL){
			...
			else if(newNode->duration < iterator->Next_TIM_Node->duration){
				break;
			}
			else{
				iterator = iterator->Next_TIM_Node;
			}
		...
		if(iterator->Next_TIM_Node != NULL){
			...
			while(Reduce_iterator != NULL){
				Reduce_iterator->duration -= newNode->duration;
