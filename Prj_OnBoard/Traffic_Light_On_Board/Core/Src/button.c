...
void button_reading(){
	for(int i = 0; i < N0_OF_BUTTONS; i++){
		...
		// update state of button and check if the button is only press once ()
		if( btn[i].debounceButton_3 == btn[i].debounceButton_1){
			if(btn[i].btn_state != btn[i].debounceButton_1){
				if(btn[i].debounceButton_1 == BUTTON_IS_RELEASED){
					if(btn[i].counterForButtonPress >= 2 && btn[i].counterForButtonPress <= 5){
						btn[i].flagpress = 1;
						btn[i].counterForButtonPress = 0;
					}
					else btn[i].counterForButtonPress = 0;
				}
			btn[i].btn_state = btn[i].debounceButton_1;
			}
			if(btn[i].btn_state == BUTTON_IS_PRESSED){
				btn[i].counterForButtonPress++;
			}
		}
